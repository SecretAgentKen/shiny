<template>
	<nav class="panel">
		<p class="panel-heading">
			Ignore/Retain the following sites for cookies, cache, and storage
		</p>
...
export default {
	...
	methods: {
		...
		loadOrigins() {
			chrome.storage.sync.get("excludes", (data) => {
				this.originObjects = data.excludes.map((origin) => {
		...
		removeOrigins() {
			let data = { excludes: [] };
			data.excludes = this.originObjects
				.filter((obj) => {
					return !obj.selected;
				})
				.map((obj) => {
